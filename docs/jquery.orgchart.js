 ...
 */
'use strict';
var orgchart_children_visible=false;
(function (factory) {
  ...
  OrgChart.prototype = {
    ...
    showChildren: function ($node) {
      ...
      //////the below one was added to make sure the line get visible again 20181025
      $animatedNodes.closest('table').closest('tr').prevAll('.lines').css('visibility', 'visible');
    ...
    hideSiblings: function ($node, direction) {
      ...
      $animatedNodes.eq(0).one(
        'transitionend',
        { 'node': $node, 'nodeContainer': $nodeContainer, 'direction': direction, 'animatedNodes': $animatedNodes, 'lines': $lines },
    ...
    loadNodes: function (rel, urls, $edge) {
      var that = this;
      var opts = this.options;
      $.ajax({ 'url': urls, 'dataType': 'json' })
      .done(function (data) {
    ...
    topEdgeClickHandler: function (event) {
      ...
      if (parentState.exist) {
        ...
        if (this.startLoading($topEdge)) {
          var opts = this.options;
          var urls = $.isFunction(opts.ajaxURL.parent) ? opts.ajaxURL.parent($node.data('nodeData')) : opts.ajaxURL.parent + $node[0].id;
          this.loadNodes('parent', urls, $topEdge);
        }
      }
    },
    //
    bottomEdgeClickHandler: function (event) {
      ...
      */
        ...
        */
        if(orgchart_children_visible==false){ // show the descendants
          this.showChildren($node);
          orgchart_children_visible=true;
        }else{
          this.hideChildren($node);
          orgchart_children_visible=false;
        }
      } else { // load the new children nodes of the specified node by ajax request
        if (this.startLoading($bottomEdge)) {
          var opts = this.options;
          var urls = $.isFunction(opts.ajaxURL.children) ? opts.ajaxURL.children($node.data('nodeData')) : opts.ajaxURL.children + $node[0].id;
          this.loadNodes('children', urls, $bottomEdge);
        }
      }
    ...
    hEdgeClickHandler: function (event) {
      ...
      if (siblingsState.exist) {
        ...
        if (this.startLoading($hEdge)) {
          ...
          var urls = (this.getNodeState($node, 'parent').exist) ?
          ...
          this.loadNodes('siblings', urls, $hEdge);
        }
      }
    },
    //
